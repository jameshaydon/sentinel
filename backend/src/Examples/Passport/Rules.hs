...
import Sentinel.Solver.Types (Proof (..), scalarToText)
...
brit person = withRule ("brit(" <> scalarToText person <> ")") do
  ...
  pure $ RuleApplied "brit" [isBritProof, proof]

-- | British by birth: born in UK or born abroad.
byBirth :: PersonId -> SolverM Proof
byBirth person = withRule ("by_birth(" <> scalarToText person <> ")") do
  ifThenElse
...
britBornInUk person = withRule ("born_in_uk(" <> scalarToText person <> ")") do
...
britBornInUkViaParent person = withRule ("born_in_uk_via_parent(" <> scalarToText person <> ")") do
...
britBornAbroad person = withRule ("born_abroad(" <> scalarToText person <> ")") do
  viaParent person \parent ->
    britOtbd parent
      `orElse` (do
...
britOtbd person = withRule ("brit_otbd(" <> scalarToText person <> ")") do
  isBritOtbdProof <- askable "possibly_brit_otbd" [person]
  proof <-
    askable "naturalised" [person]
      `orElse` britOtbdUkBorn
      `orElse` bornCrownService person
  pure $ RuleApplied "brit_otbd" [isBritOtbdProof, proof]
  where
    britOtbdUkBorn = do
      proof <- andAll
        [ askable "born_in_uk" [person],
...
bornCrownService person = withRule ("crown_service(" <> scalarToText person <> ")") do
