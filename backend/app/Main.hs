...
usageDoc =
  vsep
    [ "Usage: sentinel <example>",
      "",
      "Available examples:",
      indent 2 $ vsep [exampleLine airLogicExample, exampleLine airCanadaExample],
      "",
      "Example: sentinel airlogic"
    ]
  where
    exampleLine :: Example db -> Doc Ann
    exampleLine ex = pretty ex.name <+> "-" <+> pretty ex.description
