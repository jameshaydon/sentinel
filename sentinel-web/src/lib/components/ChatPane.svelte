...
<div class="flex h-full flex-col bg-white">
	...
	<div bind:this={chatContainer} class="flex-1 overflow-y-auto p-4">
		{#each sentinel.chatMessages as msg (msg.id)}
			{#if msg.isDebug}
				<div class="mb-2">
					<details class="rounded border border-gray-200 bg-gray-50 text-xs">
						<summary class="cursor-pointer px-2 py-1 text-gray-500 select-none">
							{truncate(msg.content, 80)}
						</summary>
						<pre class="overflow-x-auto p-2 whitespace-pre-wrap text-gray-600">{msg.content}</pre>
					</details>
				</div>
			{:else if msg.isError}
